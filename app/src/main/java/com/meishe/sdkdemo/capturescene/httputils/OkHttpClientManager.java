...
public class OkHttpClientManager {
    ...
    /**
     * 断点续传
     * Download files brokenly
     * @param url         url
     * @param destFileDir 本地路径;Local path
     * @param start       开始的位置;Starting position
    ...
    private void _downloadAsyn(final String url, final String destFileDir, long start, long total, final ResultCallback callback) {
        ...
        * Determine the download range, add this header, and the server can skip the downloaded part
        * */
     ...
     * @param url         地址；url
     * @param destFileDir 本地文件存储路径；Local file storage path
     * @param callback   test download path : http://yixin.dl.126.net/update/installer/yixin.apk
     */
    ...
    /**
     * 模拟表单上传文件：通过MultipartBody，mdiatype这个需要和服务端保持一致
     * Upload file in simulation form: through MultipartBody, mdiatype needs to be consistent with the server
     */
    private void doUpload(String url, File file, final ResultCallback callBack) {
        ...
        * Wrapping the request body into CountingRequestBody
        * */
        ...
        /*
        * 将Request封装为Call对象
        * Encapsulating Request as a Call object
        * */
        Call call = mOkHttpClient.newCall(request);
        /*
        * 执行Call
