...
import static com.meishe.sdkdemo.mimodemo.common.Constants.VIDEOVOLUME_DEFAULTVALUE;
import static com.meishe.sdkdemo.mimodemo.common.utils.asset.NvAsset.AspectRatio_NoFitRatio;
...
public class TimelineData {
    ...
    private int m_makeRatio = AspectRatio_NoFitRatio;//默认值，不作比例适配
    ...
    public void addClip(FxClipInfo clipInfo) {
    ...
    public synchronized ArrayList<CompoundCaptionInfo> getCompoundCaptionArray() {
        return m_compoundCaptionArray;
    }

    public synchronized void setCompoundCaptionArray(ArrayList<CompoundCaptionInfo> compoundCaptionArray) {
    ...
    public ArrayList<CompoundCaptionInfo> cloneCompoundCaptionData() {
        ...
        for (int index = 0; index < m_compoundCaptionArray.size(); index++) {
            CompoundCaptionInfo captionInfo = m_compoundCaptionArray.get(index);
            if (captionInfo == null) {
                newList.add(new CompoundCaptionInfo());//占位
                continue;
            }
            CompoundCaptionInfo newCaptionInfo = captionInfo.clone();
            newList.add(newCaptionInfo);
        }
       /* for(CompoundCaptionInfo captionInfo : getCompoundCaptionArray()) {
            CompoundCaptionInfo newCaptionInfo = captionInfo.clone();
            newList.add(newCaptionInfo);
        }*/
        return newList;
