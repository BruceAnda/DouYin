...
public class NvsTimelineEditor extends RelativeLayout {
    ...
    public double m_pixelPerMicrosecond = 0D;//每微秒显示的像素值
    ...
    //初始化TimelineEditor
    public void initTimelineEditor(ArrayList<NvsMultiThumbnailSequenceView.ThumbnailSequenceDesc> sequenceDesc, final long timelineDuration,final long trimInPoint) {
        ...
        //设置起始位置，需要从trimInPoint开始
        new Handler().postDelayed(new Runnable() {
            @Override
            public void run() {
                long pixel = (long) (m_pixelPerMicrosecond*trimInPoint);
                m_multiThumbnailSequenceView.scrollBy((int)pixel,0);
            }
        },100);
    ...
    public void setVideoTrimImPoint(long trimImPoint){
        long pixel = (long) (m_pixelPerMicrosecond*trimImPoint);
